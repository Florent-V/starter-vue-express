...
<template>
  <div class="container mx-auto p-6">
    <div class="bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white p-6 rounded-lg shadow-md">
      ...
      <p class="text-gray-600 dark:text-gray-400 mt-4">Date de sortie : {{ new Date(product.releaseDate).toLocaleDateString() }}</p>

      <div class="flex mt-6 space-x-4">
        <RouterLink :to="`/product/${product.id}/edit`" class="bg-blue-500 hover:bg-blue-700 text-white py-2 px-4 rounded">Éditer</RouterLink>
        <button @click="deleteProduct" class="bg-red-500 hover:bg-red-700 text-white py-2 px-4 rounded">Supprimer</button>
